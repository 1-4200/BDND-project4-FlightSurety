...
contract FlightSuretyApp {
    ...
    address[] multiCalls = new address[](0);    // To track all the addresses that have called a function that needs to implement muti-party consensus
    ...
    *
    */   
    function registerAirline(string name, address wallet )external isAllowedToRegisterAirline returns(bool success, uint256 votes){
        ...
        if (airlineCount % 2 != 0) {
            votes_needed = votes_needed.add(1);                 //make an adjustment in case of odd number
        }
        if (airlineCount < 4) {                                 //register a new airline until there are at least four airlines registered
            ...
            if (multiCalls.length >= votes_needed) {            //check if airlines has multi-party consensus of 50% of registered airlines
                flightSuretyData.registerAirline(name, wallet);
                multiCalls = new address[](0);                  //re-initialize multi calls
                return (true, votes_needed);
            }
            return (false, multiCalls.length);
        }
    }
    ...
    * This is the function that is intended to be triggered when the Oracle comes back with a result
    * and it has to decide where things go from there. (Be careful of status 20 )
    */
